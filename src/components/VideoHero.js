...
export default function VideoHero({ 
  ...
  fallbackImage, 
  onVideoPlay,
  mainHeading = "The Art Of Aesthetic Innovation",
  ...
  const handleWatchVideo = () => {
    if (onVideoPlay) {
      ...
      // Default behavior - could open a modal, navigate to video page, etc.
  ...
  return (
    <div className="relative w-full max-w-[1536px] mx-auto overflow-hidden">
      ...
      <div className="relative w-full h-[400px] lg:h-auto lg:aspect-[19/9] lg:max-h-[800px]">
        {videoSrc && !videoError ? (
          <>
            ...
            {!isVideoLoaded && (
              <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-gray-800 via-gray-700 to-gray-900 relative overflow-hidden">
                <div className="absolute inset-0 opacity-20">
                  <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-white rounded-full opacity-10 animate-pulse"></div>
                  <div className="absolute top-3/4 right-1/4 w-24 h-24 bg-white rounded-full opacity-5 animate-pulse"></div>
                  <div className="absolute bottom-1/4 left-1/3 w-40 h-40 bg-white rounded-full opacity-8 animate-pulse"></div>
                </div>
      ...
      <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4">
        {/* Main Heading */}
        <h1 className="font-serif font-bold text-white text-4xl md:text-5xl lg:text-6xl xl:text-7xl leading-tight mb-6 drop-shadow-lg">
          {mainHeading.split(' ').map((word, index) => (
            <span key={index}>
              ...
              {index < mainHeading.split(' ').length - 1 && ' '}
              {(index === 1 || index === 2) && <br />}
            </span>
          ))}
        </h1>
        ...
        {/* Subtitle */}
        <h2 className="text-white text-sm md:text-base lg:text-lg font-light tracking-wider mb-2 uppercase drop-shadow-md">
          {subtitle}
        </h2>
        
        {/* Description */}
        <p className="text-white text-xs md:text-sm lg:text-base font-light tracking-wider mb-8 uppercase drop-shadow-md">
          {description}
        </p>
        ...
        <button
          ...
          className="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 bg-white bg-opacity-90 hover:bg-opacity-100 rounded-full transition-all duration-300 group shadow-lg hover:shadow-xl focus:outline-none focus:ring-4 focus:ring-white focus:ring-opacity-30"
        ...
        {/* Button Label */}
        <span className="text-white text-sm font-light mt-4 tracking-wide drop-shadow-md">
